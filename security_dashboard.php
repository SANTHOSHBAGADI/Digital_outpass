...
// Helper function to check if a log entry exists for a request
function logEntryExists($conn, $request_id) {
...
// Handle verification of requests
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['verify'])) {
    $request_id = $_POST['request_id'];
    $student_id = $_POST['student_id'];
    $verified_at = date('Y-m-d H:i:s');

    // Update the outpass request status to 'verified'
    $query = "UPDATE outpass_requests SET status = 'verified', security_verified_at = ? WHERE id = ?";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "si", $verified_at, $request_id);
    if (mysqli_stmt_execute($stmt)) {
        // Check if a log entry exists for this request
        if (logEntryExists($conn, $request_id)) {
            // Update the existing log entry with the verify time
            $log_query = "UPDATE log_book SET verify_time = ? WHERE request_id = ?";
            $log_stmt = mysqli_prepare($conn, $log_query);
            mysqli_stmt_bind_param($log_stmt, "si", $verified_at, $request_id);
        } else {
            // Insert a new log entry with the verify time
            $log_query = "INSERT INTO log_book (request_id, student_id, verify_time) VALUES (?, ?, ?)";
            $log_stmt = mysqli_prepare($conn, $log_query);
            mysqli_stmt_bind_param($log_stmt, "iss", $request_id, $student_id, $verified_at);
        }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['check_out'])) {
    ...
    if (logEntryExists($conn, $request_id)) {
        ...
        $log_query = "UPDATE log_book SET check_out_time = ? WHERE request_id = ?";
        $log_stmt = mysqli_prepare($conn, $log_query);
        mysqli_stmt_bind_param($log_stmt, "si", $timestamp, $request_id);
    } else {
        // Insert a new log entry
        $log_query = "INSERT INTO log_book (request_id, student_id, check_out_time) VALUES (?, ?, ?)";
        $log_stmt = mysqli_prepare($conn, $log_query);
...
// Handle search for approved requests
$approved_search = '';
if (isset($_POST['approved_search'])) {
    $approved_search = trim($_POST['approved_search']);
}
...
if (!empty($approved_search)) {
    $count_query .= " AND r.student_id LIKE ?";
}
$count_stmt = mysqli_prepare($conn, $count_query);
if (!empty($approved_search)) {
    $search_term = "%$approved_search%";
...
// Fetch approved requests with pagination
$query = "SELECT r.*, s.name, s.parent_name, s.parent_phone, s.parent_email 
          FROM outpass_requests r 
          JOIN students s ON r.student_id = s.student_id 
          WHERE r.status = 'approved'";
if (!empty($approved_search)) {
...
<head>
    ...
    <style>
        * {
        ...
        .request button[name="check_out"] {
            background-color: #28a745;
            color: #fff;
        }
        .request button[name="check_out"]:hover {
            background-color: #218838;
            transform: scale(1.05);
        }
        .request button[name="verify"] {
            background-color: #007bff;
            color: #fff;
        }
        .request button[name="verify"]:hover {
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <a href="log_book.php"><i class="fas fa-book"></i> Log Book</a>
    ...
    <div class="main-content">
        <div class="container">
            <h1><i class="fas fa-user-lock"></i> Security Dashboard</h1>
            ...
            <form method="POST" class="search-form">
                <div class="search-container">
                    <input type="text" name="approved_search" placeholder="Search by Student ID" value="<?php echo htmlspecialchars($approved_search); ?>">
                    <button type="submit"><i class="fas fa-search"></i> Search</button>
                </div>
            </form>
            <?php
            if (!$result) {
                echo "<p class='error'><i class='fas fa-exclamation-circle'></i> Error fetching approved requests: " . mysqli_error($conn) . "</p>";
            } else {
                if (mysqli_num_rows($result) == 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        <form method="POST" style="display: inline-block;">
                            ...
                            <button type="submit" name="check_out" value="check_out"><i class="fas fa-sign-out-alt"></i> Check Out</button>
                        </form>
                        <form method="POST" style="display: inline-block;">
                            <input type="hidden" name="request_id" value="<?php echo $row['id']; ?>">
                            <input type="hidden" name="student_id" value="<?php echo $row['student_id']; ?>">
                            <button type="submit" name="verify" value="verify"><i class="fas fa-check"></i> Verify</button>
                        </form>
                        <?php
                        echo "</div>";
                    }
